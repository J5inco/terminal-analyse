...
export default async function handler(req, res) {
  ...
  if (mode === 'financials') {
    try {
      ...
      const summaryUrl = `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(ticker)}?modules=incomeStatementHistory,cashflowStatementHistory,balanceSheetHistory,earningsHistory${crumbParam}`;
  ...
  try {
    ...
    try {
      ...
      const summaryUrl = `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(ticker)}?modules=${modules}${crumbParam}`;
      const summaryRes  = await fetch(summaryUrl, { headers: authHeaders });
      const summaryData = await summaryRes.json();
      const s = summaryData?.quoteSummary?.result?.[0];
      if (s) {
